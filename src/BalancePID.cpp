...
void BalancePID::loadGains(Preferences& prefs) {
    ...
    kp = prefs.getFloat("pid_kp", 3.5f);
    ki = prefs.getFloat("pid_ki", 0.0f);
    kd = prefs.getFloat("pid_kd", 0.10f);
    prefs.end();
}
...
void BalancePID::compute() {
    ...
    // SetpointAccum: previous output slowly drifts the PID target angle.
    // This lets the gimbal return toward centre between corrections without
    // the PID fighting itself (same trick as XRobots ControlMomentGyros V3).
    ...
    float error = (setpoint + setpointAccum) - dispRoll;

    _integral += error * dt;
    _integral  = constrain(_integral, -ILIMIT, ILIMIT);

    // Derivative on measurement — avoids spike when setpoint changes
    float dMeas = (dispRoll - _prevMeas) / dt;
    _prevMeas   = dispRoll;

    // Output in degrees of servo deflection from default — sent directly as position
    output = kp * error + ki * _integral - kd * dMeas;
    output = constrain(output, -OUTPUT_MAX_DEG, OUTPUT_MAX_DEG);
    _prevOutput = output;

    _servo.moveTo(_servo.defaultPos - (int)(output / 0.088f));
}
